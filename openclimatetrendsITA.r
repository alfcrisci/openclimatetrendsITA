...
##################################################################################################
# Trend calculation of mean air temperature 786 is numer of months for 65.5 years 1950 - 2015

tg_ita_T <- TrendRaster(tg_ita,start=c(1950,1), freq=12,method="SeasonalAdjusted", breaks=0, funSeasonalCycle=MeanSeasonalCycle)

png("trend_T.png")
plot(tg_ita_T[["SlopeSEG1"]]*786,col=brewer.pal(8,"YlOrRd"))
plot(italia_bound_region,add=T)
dev.off()

##################################################################################################
# Trend calculation of monthly cumulated rainfall 786 is numer of months for 65.5 years 1950 - 2015
...
png("rain_T.png")
plot(rain_ita_T[["SlopeSEG1"]]*786,col=brewer.pal(8,"RdBu"))
plot(italia_bound_region,add=T)
dev.off()
